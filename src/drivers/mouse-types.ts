...
export interface MouseStatus {
   ...
   * value and bounds are raw device codes; the millimetre figures are what
   * the control labels. Drivers that only offer the stops leave this undefined,
   * and a driver that sets it should still fill `liftOffDistance` with the
   * nearest stop so anything reading the coarse field keeps working.
   */
  liftOffScale?: {
    value: number;
    min: number;
  ...
  supportedLiftOffDistances?: Array<
    NonNullable<MouseStatus["liftOffDistance"]>
  >;
